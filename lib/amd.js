...
| Follow support channel below 👇 
------------✘
| https://whatsapp.com/channel/0029VaZAY2P4yltOvNaegY2w
-----------✘
| Developer: https://wa.me/2348102487241
| Co-Developer: https://wa.me/2347080968564
-----------✘
            `
      try {
...
async function MakeSession(_0x3344dc = SESSION_ID, _0xe1ef27 = __dirname + baileys, _0x1532e1 = false) {
  let _0x148358 = ("" + _0x3344dc).replace(/^SESSION_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/^SESSION_ID_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/^ASTA_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/Asta;;;/gi, "").replace(/Astro;;;/gi, "").replace(/EDNUT-MD;;;/gi, "").trim();
