  ...
  smd(
    ...
    async (message, input) => {
      try {
        ...
        return await message.send(`*Ednut bard ai:*\n ${result}`, { quoted: message });
      } catch (error) {
...
/*  smd({
  ...
  smd({
    ...
    try {
      ...
      const { result } = res;
      const Ednut = "Ednut-Md ɢᴇᴍɪɴɪ ᴀɪ"
      const tbl = "```";
    ...
    async (_0x2acf30) => {
      try {
        if (
          ...
          if (
            _0x2acf30.reply_text.includes("EDNUT-MD • ᴀɴɴᴏɴʏᴍᴏᴜs ᴍsɢ") &&
            _0x2dfb59[0].includes("Ednut-Md • ᴀɴɴᴏɴʏᴍᴏᴜs ᴍsɢ") &&
