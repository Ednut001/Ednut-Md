  ...
  async function updateHerokuApp() {
    try {
      ...
      if (commits.total === 0) {
        ...
        console.log("Update Detected, trying to update make your bot uptodated!");
        ...
        return "Bot on latest version. Restarting hold.";
      }
    } catch (e) {
      print(e);
    ...
    async (citel, text) => {
      try {
        ...
        await citel.bot.sendMessage(
          citel.chat,
          { text: update.replace(/SuhailTechIMd/, "Ednut-Md") },
          { quoted: citel }
        );
        if (
          text == "start" &&
