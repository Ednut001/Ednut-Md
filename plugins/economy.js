...
smd({
    ...
    async(message) => {
      try{
       ...
       const balance  = await eco.balance(users, "Ednut")
...
await message.reply('*What are you trying to do📉*.')
   ...
   async(message,match) => {
     ...
     //  let users = message.mentionedJid ? message.mentionedJid[0] : message.msg.contextInfo.participant || false;
       ...
       const deposit = await eco.deposit(message.sender, "Ednut", d);
       const balance = await eco.balance(message.sender, "Ednut")
...
async(message,match) => {
  try{
   ...
   if(!user) return message.reply('Please give me any user🤦‍♂️.');
       ...
       const balance = await eco.balance(user1, "Ednut");
       let a = (balance.wallet) < parseInt(word)
       //Returns wallet, bank, and bankCapacity. Also creates a USer if it doesn't exist.
       if(a == true) return message.reply("you dont have sufficient money to transfer👎");

       const deduct = await eco.deduct(user1, "Ednut", value[0]);
       const give = await eco.give(user2, "Ednut", value[0]);
  ...
  // return await Aviator.bot.sendButtonText(message.chat, `*📠 Transaction successful of ${value[0]} 💰*`, `${Config.ownername.split(' ')[0]}-Economy Version: 0.0.6`, message);
   ...
   async(message) => {
     try{
       ...
       let mongoschemas = zerogroup.economy || "false";
       if (mongoschemas == "false") return message.reply("*🚦Economy* is not active in current group.");
        const balance = await eco.balance(message.sender,"Ednut"); //Returns wallet, bank, and bankCapacity. Also creates a USer if it doesn't exist.
   ...
   async(message,match) => {
     try{
       if(!message.isCreator) return message.reply(`*_Hey Master, only my owner can give money!_*`)
        ...
        await eco.give(users, "Ednut", parseInt(match.split(' ')[0]));
   ...
   if(!users) return message.reply('Please give me user to rob.')
       ...
       const user2 = users
       const k = 1000
       const balance1  = await eco.balance(user1, "Ednut")
   const balance2  = await eco.balance(user2, "Ednut")
   ...
   switch (random) {
     ...
     const deduff = Math.floor(Math.random() * 1000)	    
         await eco.deduct(user2, "Ednut", deduff);
         await eco.give(message.sender, "Ednut", deduff);
       ...
       case 'caught':
          ...
          await eco.deduct(user1, "Ednut", rmoney);
...
///.mentionedJid[0] ? m.mentionedJid[0] : m.sender
    const balance = await eco.balance(user, "Ednut");
  ...
  message.reply(media,{packname:"Ednut",author:'Economy'},"sticker")
    ...
    if (!match) return message.reply(`Example:  ${prefix}gamble 100 direction(left,right,up,down)`);
          ...
          if ( r == opp){
          let give = await eco.give(user , "Ednut", twice);    //message.react('⭐️')
   ...
   //return await Aviator.bot.sendButtonText(message.chat, `*📈 You won 🪙${twice}*`, `${Config.ownername.split(' ')[0]}-Economy \n Version: 0.0.6`, message);
                 ...
                 let deduct = await eco.deduct(user, "Ednut", texts[0]);
   ...
   async(message,match) => {
     try{
       ...
       if (today.getDay() == 6 || today.getDay() == 5 || today.getDay() == 0){
           ...
           const user = message.sender
           const k = 100
           const balance1  = await eco.balance(user,"Ednut")
...
if(value<=balance.wallet){
   ...
   if ((f1 !== f2) && f2 !== f3){
       const deduct1 = await eco.deduct(user, "Ednut", deduff);
       return message.reply(`${mess1}\n\n*Big Lose -->* _🪙${deduff}_`)
    }
    else if ((f1 == f2) && f2 == f3){
       const give1 = await eco.give(user, "Ednut", deduff/2);
       return message.reply(`${mess2}\n*_Little Jackpot -->* _🪙${deduff/2}_`)
    }
    else if ((f1 == f2) && f2 !== f3){
       const give2 = await eco.give(user, "Ednut", deduff);
       ...
       const give4 = eco.give(user,"Ednut", deduff);
       return message.reply(`${mess3}\n\n*Small Win -->* _🪙${deduff}_`)
    }
    else if ((f1 == f2) && (f2 == f3) && (f3 == f4)){
       const give5 = eco.give(user,"Ednut", deduff*20);
